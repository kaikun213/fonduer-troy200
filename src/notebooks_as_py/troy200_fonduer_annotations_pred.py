...
get_ipython().system(' dropdb -h postgres -h postgres -h postgres -h postgres -h postgres --if-exists troy200_col_row_data_pred')
get_ipython().system(' createdb -h postgres -h postgres -h postgres -h postgres -h postgres troy200_col_row_data_pred')
...
ATTRIBUTE = "troy200_col_row_data_pred"

DB_USERNAME = 'user'
DB_PASSWORD = 'venron'
conn_string = f'postgresql://{DB_USERNAME}:{DB_PASSWORD}@postgres:5432/{ATTRIBUTE}'
    
docs_path = 'data/pred/html/'
pdf_path = 'data/pred/pdf/'
...
def get_label_matcher(label):
    def label_matcher(mention):
        ...
        # if no cell-type prediciton, then still use as mention in all cases
        if (len([x for x in html_attrs if 'color' in x]) == 0):
            return True
        return {
